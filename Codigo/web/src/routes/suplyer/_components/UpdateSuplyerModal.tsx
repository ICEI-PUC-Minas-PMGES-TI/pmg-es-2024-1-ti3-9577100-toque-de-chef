...
export const UpdateSuplyerModal = () => {
  const queryClient = useQueryClient();
  const [updateSuplyerModal, setUpdateSuplyerModal] =
    useSearchParam("updateSuplyerModal");
  const {
  ...
  const onSubmitUpdate: SubmitHandler<z.infer<typeof schemaEdit>> = async (
    ...
    setUpdateSuplyerModal(undefined);
  ...
  return (
    <Modal
      show={Boolean(updateSuplyerModal)}
      onHide={() => setUpdateSuplyerModal(undefined)}
    >
      ...
      <Modal.Body>
        <Form onSubmit={handleSubmitUpdate(onSubmitUpdate)}>
          ...
          <div className="d-flex gap-2 mt-2">
            <Button
              variant="secondary"
              onClick={() => setUpdateSuplyerModal(undefined)}
            >
              Fechar
            </Button>
